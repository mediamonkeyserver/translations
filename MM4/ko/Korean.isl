...
InvalidParameter=명령줄에 잘못된 매개 변수:%n%n%1
SetupAlreadyRunning=설치 프로그램이 이미 실행 중입니다.
WindowsVersionNotSupported=이 프로그램은 컴퓨터가 실행중인 Windows 버전을 지원하지 않습니다.
...
CannotInstallToNetworkDrive=설치 프로그램이 네트워크 드라이브에 설치할 수 없습니다.
CannotInstallToUNCPath=설치 프로그램이 UNC 경로에 설치할 수 없습니다.
...
ApplicationsFound=설치 프로그램이 업데이트하려는 파일을 다음 응용 프로그램에서 사용하고 있습니다. 이러한 응용 프로그램을 자동을 닫고 설치를 계속하는 것이 좋습니다.
ApplicationsFound2=설치 프로그램이 업데이트하려는 파일을 다음 응용 프로그램에서 사용하고 있습니다. 이러한 응용 프로그램을 자동을 닫고 설치를 계속하는 것이 좋습니다. 설치가 완료되면, 설치 프로그램이 이 응용 프로그램의 다시 시작을 시도합니다.
CloseApplications=이 응용 프로그램을 자동으로 닫기(&A)
DontCloseApplications=이 응용 프로그램을 자동으로 닫지 않기(&N)
...
ShutdownBlockReasonInstallingApp=%1 설치.
ShutdownBlockReasonUninstallingApp=%1 설치 제거.
...
AutoStartProgram=자동으로 %1을(를) 시작합니다
